...
git_check_untracked() {
    git_untracked || { echo "[git] No untracked files"; return 0; }
    [ $opt_force -eq 1 ] && echo "[warn-git] You have untracked files" && return 0
    git status
    echo "[error-git] You have untracked files, use --force" && exit 1 
}
